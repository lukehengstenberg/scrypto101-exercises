...
mod radiswap_module {
    struct Radiswap {
        /// A vault containing pool reserves of token A.
        vault_a: Vault,
        /// A vault containing pool reserves of token B.
        vault_b: Vault,
    ...
    impl Radiswap {
        ...
        pub fn instantiate_radiswap(
            ...
            // Create a badge which will be given the authority to mint the pool unit tokens.
            let pool_units_minter_badge: Bucket = ResourceBuilder::new_fungible()
                .divisibility(DIVISIBILITY_NONE)
                .metadata("name", "LP Token Mint Auth")
                .mint_initial_supply(1);

            // Create the pool unit resource along with the initial supply by the user.
        ...
        pub fn swap(&mut self, input_tokens: Bucket) -> Bucket {
            ...
            // Calculate the output amount of tokens based on the input amount and the
            // pool fees
